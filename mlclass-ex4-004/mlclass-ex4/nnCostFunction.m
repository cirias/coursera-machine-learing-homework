...
% Theta2  		10 x 26
% X		  		m  x 400
% Theta1 * [ones(m,1) X]'  	25 x m
% Theta2 * ([ones(m,1) sigmoid(Theta1 * [ones(m,1) X]')]) 10 x m
% hOfx			10 x 1
seq = [1:num_labels]';

for i = 1:m
	hOfxi = sigmoid(Theta2 * ([ones(1,1) sigmoid(Theta1 * [ones(1,1) X(i,:)]')'])');
	jOfi = -log(hOfxi') * (seq == y(i)) - log(1 - hOfxi') * (1 - (seq == y(i)));
	J = J + jOfi;
end
J = 1/m * J;
...
for i = 1:m
    ...
    delta_3 = a_3 - (seq == y(i));      % 10 x 1
    delta_2 = Theta2' * delta_3 .* [1; sigmoidGradient(z_2)];% 26 x 1
    Del_1 = Del_1 + delta_2(2:end) * a_1';
...
for i = 1:hidden_layer_size
    for j = 1:input_layer_size + 1
		if j == 1
			Theta1_grad(i, j) = 1/m * Del_1(i, j);
		else
			Theta1_grad(i, j) = 1/m * Del_1(i, j) + lambda/m * Theta1(i, j);
		endif
    end
end
...
for i = 1:num_labels
    for j = 1:hidden_layer_size + 1
		if j == 1
			Theta2_grad(i, j) = 1/m * Del_2(i, j);
		else
			Theta2_grad(i, j) = 1/m * Del_2(i, j) + lambda/m * Theta2(i, j);
		endif
    end
end
